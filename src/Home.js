...
const Home = () => {
  ...
  const handleSubmit = async () => {
    try{
      ...
      const date = {
        ...postData,
        releaseDate: new Date(postData.releaseDate).toLocaleDateString("en-IN", options)
      };
            ...
            <Box sx={{ display: "flex", flexWrap: "wrap" }}>
              ...
              <Box className="sort">
                <Select id="sort" value={sort} onChange={handleSort}>
                  <MenuItem id="release-date-option" selected value="releaseDate">Release Date</MenuItem>
                  <MenuItem id="view-count-option" value="viewCount">View Count</MenuItem>
                </Select>
              </Box>
            </Box>
          </Grid>
          <Grid item xs={12} m={1}>
            <Box sx={{ display: "flex", flexWrap: "wrap" }}>
              {ageGroup.map((item) => (
                <Box
                  className={
                    age.includes(item.value) ? "content-rating-btn active" : "content-rating-btn"
                  }
                  key={item.value}
                  id={item.value}
                  component="span"
